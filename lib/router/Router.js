...
var doViewMap = function (pathname, params, callback) {
	...
	if (router && Utils.isFunction(router.handler)) {
		var scope = newScope(this.context, session, {pathname: pathname, params: viewParams});
		result = router.handler.call(scope, pathname, viewParams, _callback);
	}
	else if (Utils.isFunction(this.adapter.view)) {
		result = this.adapter.view(pathname, params.params, _callback);
	}
