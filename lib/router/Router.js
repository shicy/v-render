...
var doApiMap = function (pathname, params, callback) {
	...
	if (StringUtils.isBlank(apiName)) {
		...
		apiRouterHandler.call(this, session, apiName, apiParams, apiVersion, function (err, state, data) {
			if (!err) {
...
var doViewMap = function (pathname, params, callback) {
	...
	VRender.logger.debug("<Router.view>", pathname, JSON.stringify(viewParams).substr(0, 8000));
	...
	if (router && Utils.isFunction(router.handler)) {
		var scope = newScope(this.context, params.params.session, {pathname: pathname, params: viewParams});
...
var newScope = function (context, session, data) {
	...
	scope.send = function (url, params, callback) {
		return session.send(url, params, callback);
	};
