...
Router.prototype.map = function (url, params, request, response, callback) {
	// VRender.logger.debug("<Router.map>", url.pathname, JSON.stringify(params.requestData));
...
// 执行一个api
Router.prototype.doApi = function (session, apiName, apiParams, callback) {
	var params = {pathname: "/api"};
	params.params = {session: session, requestData: {n: apiName, data: apiParams}};
	doApiMap.call(this, params.pathname, params, function (err, state, data) {
		// if (!err && data && data.code == 0 && data.hasOwnProperty("data")) {
		// 	data = data.data;
		// }
		callback(err, data);
	});
...
// 文件处理
Router.prototype.getFileHandler = function (params) {
	var fileHandler = this.adapter.fileHandler;
	if (Utils.isFunction(fileHandler)) {
		return Through(function (chunk, enc, callback) {
			fileHandler(params.filepath, params, chunk, function (err, ret) {
				if (!err) {
					callback(null, ret);
				}
				else {
...
var doBefore = function (params, callback) {
	...
	var self = this;
	PluginManager.routeBefore.call(this.context, params, function (err, state, data) {
		if (arguments.length > 0) {
			resultHandler(err, state, data);
		}
		else if (Utils.isFunction(self.adapter.before)) {
			var result = false;
			result = self.adapter.before(params.pathname, params.params, function (err, state, data) {
				setTimeout(function () {
					if (result === true)
						resultHandler(err, state, data);
				});
...
var doMap = function (params, callback) {
	if (params.result) { // doBefore() 中已返回结果
		...
		var resultHandler = function (err, state, data) {
			...
			else if (err.code == VRender.RouterStatus.NOTFOUND) {
				params.result = {state: VRender.RouterStatus.NOTFOUND, data: (err.msg || err)};
				callback(false, params);
			}
			else {
				callback(err, params);
		...
		else if (pathname == "/upload") { // 上传
			doUploadMap.call(this, pathname, params, resultHandler);
		}
...
var doAfter = function (params, callback) {
	...
	var self = this;
	PluginManager.routeAfter.call(this.context, params, function (err, ret) {
		if (err) {
			resultHandler(err, ret);
		}
		else if (Utils.isFunction(self.adapter.after)) {
			var result = false;
			result = self.adapter.after(params.pathname, params.params, function (err, ret) {
				setTimeout(function () {
					if (result === true)
						resultHandler(err, ret);
				});
...
var doApiMap = function (pathname, params, callback) {
	...
	if (StringUtils.isBlank(apiName)) {
		...
		if (StringUtils.isNotBlank(apiParams) && (typeof apiParams == "string")) {
			try {
				apiParams = JSON.parse(apiParams);
			}
			catch (e) {}
		}
		apiRouterHandler.call(this, session, apiName, apiParams, apiVersion, callback);
	}
};

// 文件上传
var doUploadMap = function (pathname, params, callback) {
	var requestData = params.params.requestData || {};
	var apiName = requestData.n || requestData.name; // 接口名称
	if (StringUtils.isBlank(apiName)) {
		callback("无效的请求，参数name(或n)不能为空");
		...
		var session = params.params.session;
		var apiParams = {stream: params.request};
		var apiVersion = requestData.version;
		apiRouterHandler.call(this, session, apiName, apiParams, apiVersion, callback);
	}
};

var apiRouterHandler = function (session, apiName, apiParams, version, callback) {
	var resultHandler = function (err, state, data) {
		if (!err) {
	...
	var self = this;
	apiParams = apiParams || {};
	var scope = newScope(this.context, session, {version: version});
	PluginManager.routeApi.call(scope, apiName, apiParams, function (err, state, data) {
		if (arguments.length > 0) {
			resultHandler(err, state, data);
		}
		else {
			...
			if (router && Utils.isFunction(router.handler)) {
				...
				var result = false;
				if (Utils.isFunction(self.adapter.api)) {
					var _params = {session: session, data: apiParams, version: version};
					result = self.adapter.api(apiName, _params, function (err, state, data) {
						setTimeout(function () {
							if (result !== false)
								resultHandler(err, state, data);
						});
				...
				if (result === false)
					result = ApiFactory.do(apiName, apiParams, version, resultHandler);
				if (result === false)
...
var doFileMap = function (filepath, params, callback) {
	...
	var self = this;
	var scope = newScope(this.context, params.params.session, {});
	PluginManager.routeFile.call(scope, filepath, params, function (err, state, data) {
		if (arguments.length > 0) {
			resultHandler(err, state, data);
		}
		else if (/^\/VRender\./.test(filepath)) { // 获取框架前端静态资源
			filepath = Path.resolve(self.staticFilePath, filepath.substr(1));
			resultHandler(false, filepath, null, self.staticFilePath);
		}
		else if (/^\/VRender\/(css|js|icons)\//.test(filepath)) {
			filepath = Path.resolve(self.staticFilePath, filepath.substr(9));
			resultHandler(false, filepath, null, self.staticFilePath);
		}
		else if (/^\/VRender\/babel\/polyfill/.test(filepath)) {
			filepath = FileUtils.getModuleFilePath("@babel/polyfill/dist/polyfill.min.js");
			if (!filepath)
				resultHandler("不存在 @babel/polyfill/dist/polyfill.min.js 文件");
			else
				resultHandler(false, filepath);
		}
		else if (/^\/VRender\//.test(filepath)) { // 获取由框架生成的样式、脚本文件
			filepath = Path.resolve(self.dynamicFilePath, filepath.substr(9));
			resultHandler(false, filepath, null, self.dynamicFilePath);
		}
		else {
			var webRoot = self.httpServ.getWebRoot();
			if (/^\/webroot\//.test(filepath)) { // 指定获取 Web Service Root(WebContent) 下的资源文件
				...
				var result = false;
				if (Utils.isFunction(self.adapter.file)) {
					result = self.adapter.file(filepath, params.params, function (err, state, data) {
...
var doViewMap = function (pathname, params, callback) {
	...
	var session = params.params.session;
	VRender.logger.debug("<Router.view>", session.getUserHost(), pathname, 
		JSON.stringify(viewParams).substr(0, 8000));

	session.currentRouter = pathname;
	...
	var resultHandler = function (err, state, data) {
		...
		else if (state === VRender.RouterStatus.OK && (typeof data === "string")) {
			if (StringUtils.isBlank(data))
				callback(false, state, "");
			else {
				UIBuilder.build(self.context, data, params.params, function (err, html) {
					callback(err, VRender.RouterStatus.OK, html);
				});
			}
		}
		else {
	...
	var scope = newScope(this.context, session, {});
	PluginManager.routeView.call(scope, pathname, params, function (err, state, data) {
		if (arguments.length > 0) {
			resultHandler(err, state, data);
		}
		else {
			var result = false;
			var _callback = function (err, state, data) {
				setTimeout(function () {
					if (result === true || result === null || result === undefined) {
						if (!err && ((typeof state != "number") || arguments.length == 2)) {
							data = state;
							state = VRender.RouterStatus.OK;
						}
						resultHandler(err, state, data);
			...
			if (router && Utils.isFunction(router.handler)) {
				result = router.handler.call(scope, pathname, viewParams, _callback);
			}
			else if (Utils.isFunction(self.adapter.view)) {
				result = self.adapter.view(pathname, params.params, _callback);
			}

			if (result === false) {
				var path = (pathname === "/" && !!self.homePage) ? self.homePage : null;
				if (StringUtils.isBlank(path)) {
					router = findRouter(self.routerMap, pathname);
			...
			else if (StringUtils.isNotBlank(result) && result !== true) {
...
var findRouter = function (routers, name) {
	for (var i = 0; i < routers.length; i++) {
		...
		if (router.reg && !router.reg.test(name))
			continue;
		if (router.routers) {
			var subName = router.reg ? name.replace(router.reg, "") : name;
			if (subName) {
				if (!/^(\.|\/)/.test(subName) && router.reg)
					continue;
				if (/^\./.test(subName))
			...
			var _router = ArrayUtils.find(router.routers, function (temp) {
				if (temp.reg)
					return temp.reg.test(subName);
				return subName === "" || subName === "/" || subName === ".";
			});
			if (_router)
				return _router;
		}
...
var newScope = function (context, session, data) {
	var scope = data || {};
	scope.context = context;
	scope.session = session;
	scope.fetch = function (url, params, callback) {
		return session.fetch(url, params, callback);
	};
	scope.get = function (url, params, callback) {
		return session.get(url, params, callback);
	};
	scope.post = function (url, params, callback) {
		return session.post(url, params, callback);
	};
	scope.put = function (url, params, callback) {
		return session.put(url, params, callback);
	};
	scope.delete = function (url, params, callback) {
		return session.delete(url, params, callback);
	};
	return scope;
};
