...
var doApiMap = function (url, params, callback) {
	...
	if (StringUtils.isBlank(apiName)) {
		...
		if (Utils.isFunction(this.adapter.action)) {
			accept = this.adapter.action(apiName, params, function (state, data) {
				setTimeout(function () {
					if (accept)
						resultHandler(state, data);
				}, 0);
			});
		}
		if (!accept)
...
var doViewMap = function (url, params, callback) {
	...
	if (Utils.isFunction(this.adapter.before))
		name = this.adapter.before(name, params) || name;
	if (!/^\//.test(name))
		name = "/" + name; // 路由必须是“/”开头
	params.session.currentRouter = name;
	...
	if (Utils.isFunction(this.adapter.router)) {
		accept = this.adapter.router(name, params, path, function (state, newpath) {
			setTimeout(function () {
				if (accept)
