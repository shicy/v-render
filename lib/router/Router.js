	...
	if (StringUtils.isBlank(apiName)) {
		...
		if (router && Utils.isFunction(router.handler)) {
			...
			if (Utils.isFunction(this.adapter.api)) {
				var _params = {session: params.session, data: apiParams, version: apiVersion};
			...
			if (result === false)
				result = ApiFactory.do(apiName, apiParams, apiVersion, resultHandler);
			if (result === false)
