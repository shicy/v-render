...
var apiRouterHandler = function (session, apiName, apiParams, version, callback) {
	...
	var self = this;
	apiParams = apiParams || {};
	PluginManager.routeApi.call(this.context, session, apiName, apiParams, function (err, state, data) {
		if (arguments.length > 0) {
			resultHandler(err, state, data);
		}
		else {
			...
			if (router && Utils.isFunction(router.handler)) {
				var scope = newScope(self.context, session, {api: apiName, params: apiParams, version: version});
...
var doFileMap = function (filepath, params, callback) {
	...
	var self = this;
	PluginManager.routeFile.call(this, filepath, params, function (err, state, data) {
		if (arguments.length > 0) {
			resultHandler(err, state, data);
		}
...
var doViewMap = function (pathname, params, callback) {
	...
	PluginManager.routeView.call(this, pathname, params, function (err, state, data) {
		if (arguments.length > 0) {
			resultHandler(err, state, data);
		}
		else {
			...
			if (router && Utils.isFunction(router.handler)) {
				var scope = newScope(self.context, session, {pathname: pathname, params: viewParams});
