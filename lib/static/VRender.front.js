...
(function () {
		...
		return $.ajax({
			...
			error: function (e) {
				...
				if (e.status === 401) 
					...
					errormsg = ((errormsg && errormsg.hasOwnProperty("msg")) ? errormsg.msg : errormsg) || "服务器错误";
				}
				if (!ignoreError && Utils.isFunction(VRender.onAjaxError))
	...
	Component.load = function (api, params, callback) {
		...
		VRender.fetch(api, params, function (err, ret) {
			if (!err) {
				...
				pageInfo.total = Math.max(0, parseInt(ret && ret.total)) || 0;
				ret = (ret && ret.hasOwnProperty("data")) ? ret.data : ret;
			}
			pageInfo.page = parseInt(params.p_no) || 1;
			pageInfo.size = parseInt(params.p_size) || 20;
			if (Utils.isFunction(callback))
				callback(err, ret);
		});
		return true;
	...
	VRender.$ref = function (name, target) {
		...
		if (ref && ref.length > 0) {
			if (ref.length > 1)
				ref = ref.eq(0);
			return ref.data("mfhInstance") || Component.get(ref) || ref;
		}
		return null;
	};
