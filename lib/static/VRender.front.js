...
(function () {
	...
	window.requirejs = function (name, callback) {
        if (definitions.hasOwnProperty(name)) {
            ...
            VR.on(VRender.event_defined, function (_name, value) {
                if (name === _name) {
                    VR.off(VRender.event_defined, arguments.callee);
                    callback(value);
                }
            });
        }
	...
	Component.create = function (options, UIComponent, Holder) {
		...
		if (Utils.isNotBlank(options.clsName)) {
			...
			if (Utils.isBlank(options.name))
				target.attr("name", options.clsName);
		}
		...
		var holder = Holder && (new Holder(options));
		if (holder && Utils.isFunction(holder.render))
			holder.render($, target);

		if (UIComponent && Utils.isFunction(UIComponent))
			return new UIComponent(target, options, holder);
		return target;
	};
	...
	Component.load = function (api, params, callback) {
		...
		params = $.extend({page: 1, rows: 20}, params);
		if (params.page == 1)
			params.pageClear = true;
		...
		VRender.fetch(api, params, function (err, ret) {
			if (!err) {
				...
				if (Utils.isArray(ret))
					ret = {total: ret.length, results: ret};
				self._totalSize = parseInt(ret && ret.total) || 0;
				ret = (ret && ret.hasOwnProperty("results")) ? ret.results : ret;
			}
			var pager = Utils.isFunction(self.getPaginator) && self.getPaginator();
			if (pager && Utils.isFunction(pager.set))
				pager.set(self._totalSize, params.page, params.rows);
			if (Utils.isFunction(callback))
				callback(err, ret);
		});
	};
