...
(function () {
		...
		if (Utils.isFunction(callback)) {
			if (Utils.isBlank(pathname)) {
				...
				loadHTML(pathname, null, function (path, type, err, data) {
					callback(err, data, getLoadErrorMsg(err));
				});
			}
		}
	};
		...
		return $.ajax({
			...
			error: function (e) {
				...
				if (e.readyState == 0) {
					...
					try {
						errormsg = e.responseJSON || JSON.parse(e.responseText);
				...
				if (callback(errormsg, null, e) !== false) {
	...
	var getLoadErrorMsg = function (e) {
	...
	VRender.onInputChange = function (input, callback) {
		...
		if (Utils.isFunction(callback)) {
			...
			target.filter(":focus").trigger("focusin");
		}
		else {
			target.off("focusin");
			target.off("focusout");
			Utils.each(target, function (input) {
				var data = input.data("vrender-inputchange");
				if (data && data.timer)
					clearInterval(data.timer);
				input.removeData("vrender-inputchange");
	...
	Component.build = function (view) {
		...
		// console.log("Component.build:", components);
		return components;
	};
