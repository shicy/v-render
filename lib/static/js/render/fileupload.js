...
(function (backend) {
	...
	_Renderer.render = function ($, target) {
		...
		target.attr("opt-filter", getFilter.call(this));
		target.attr("opt-auto", this.isAutoUpload() ? "1" : "0");
		...
		if (backend) {
			...
			BaseRender.fn.renderFunction.call(this, target, "abefore", this.options.actionBefore);
		}
	};
	...
	var getAccept = function () {
		var filter = getFilter.call(this);
		if (filter) {
			if (filter == "image")
				return "image/*";
			if (filter == "audio")
