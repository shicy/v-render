...
(function (backend) {
	...
	var defaultButtons2 = [{name: "ok", label: "确定", type: "primary"}, {name: "cancel", label: "取消", type: "cancel"}];
	...
	var renderFootButtons = function ($, target, buttons) {
		...
		if (!buttons) {
			buttons = this.isRenderAsApp() ? defaultButtons2 : defaultButtons;
		...
		if (buttons && buttons.length > 0) {
			...
			Utils.each(buttons, function (button) {
				...
				if (backend)
					new UIButton(self.context, button).render(btn);
				else
	...
	var getDialogView = function (target) {
