...
(function (backend) {
	...
	_Renderer.render = function ($, target) {
		...
		if (this.isRangeDate()) {
			...
			if (date) {
				target.attr("opt-start", Utils.toDateString(date[0], "yyyy-MM-dd"));
				target.attr("opt-end", Utils.toDateString(date[1], "yyyy-MM-dd"));
			}
		}
		else if (date) {
	...
	var renderTables = function ($, target) {
		...
		var renderTable = function (date) {
			var tableContainer = $("<div class='table'></div>").appendTo(tables);
	...
	var renderPickerDate = function ($, target, table, pickerDate, selectedDate) {
		...
		table.find(".month").text(cn_month[m] + "月");
		...
		var tbody = table.find("table.body tbody");
		while (true) {
	...
	var checkRangeHeadBtns = function ($, target, start, end) {
		...
		var tstart = start.getFullYear() * 100 + start.getMonth();
		var tend = end.getFullYear() * 100 + end.getMonth();
		if (start >= end - 1)
