...
(function (backend) {
	...
	var ButtonRenderer = backend ? require("./button") : VRender.Component.Render.button;
	...
	var renderFooter = function ($, target) {
		if (this.isRangeDate()) {
			...
			if (this.isRenderAsApp()) {
				var btn = $("<div class='okbtn'></div>").appendTo(footer);
				new ButtonRenderer(this.context, {label: "使用日期", type: "ok"}).render($, btn);
			}
			else {
				footer.append("<div class='vals'><span class='s'>-</span> - <span class='e'>-</span></div>");
				var buttons = $("<div class='btns'></div>").appendTo(footer);
				var okbtn = $("<div class='okbtn'></div>").appendTo(buttons);
				new ButtonRenderer(this.context, {label: "确定", type: "ok"}).render($, okbtn);
				var cancelbtn = $("<div class='cancelbtn'></div>").appendTo(buttons);
				new ButtonRenderer(this.context, {label: "取消", type: "cancel"}).render($, cancelbtn);
			}
		}
	};
	...
	if (backend) {
		...
		// Renderer.renderHeaderDate = renderHeaderDate;
		// Renderer.renderTableDate = renderTableDate;
		// Renderer.renderFooterDate = renderFooterDate;
