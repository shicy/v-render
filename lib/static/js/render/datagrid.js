...
(function (backend) {
	...
	_Renderer.getSearcher = function () {
	...
	var renderItems = function ($, target, datas, indexs, selectedIndexs) {
		...
		Utils.each(datas, function (data, i) {
			...
			if (item) {
				...
				self._renderOneItem($, item, data, index);
		...
		// setTimeout(function () {
		// 	self._render_items = null;
		// }, 0);
	};

	var renderRow = function ($, row, columns, data, rowIndex) {
	...
	var renderOthers = function ($, target, columns) {
		if (backend) {
			...
			BaseRender.fn.renderFunction(target, "hrender", this.getHeadRenderer());
			BaseRender.fn.renderFunction(target, "crender", this.getColumnRenderer());
			BaseRender.fn.renderFunction(target, "erender", this.getExpandRenderer());
			BaseRender.fn.renderFunction(target, "rstyle", this.getRowStyleFunction());
			BaseRender.fn.renderFunction(target, "cstyle", this.getCellStyleFunction());

			var searcher = this.getSearcher();
			if (searcher) {
				if (typeof searcher == "string")
					target.attr("opt-searcher", Utils.trimToNull(searcher));
				else if (Utils.isFunction(searcher.getViewId))
					target.attr("opt-searcher", "[vid='" + searcher.getViewId() + "']");
			}
		}
	};
	...
	if (backend) {
		...
		Renderer.getFormatColumns = getFormatColumns;
		Renderer.getSortFunction = getSortFunction;
		Renderer.getFilterFunction = getFilterFunction;
		Renderer.renderHeader = renderHeader;
		Renderer.renderItems = renderItems;
		Renderer.renderCell = renderCell;
		Renderer.getColumnValueSet = getColumnValueSet;
		Renderer.renderToolDropdown = renderToolDropdown;
