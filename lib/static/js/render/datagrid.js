...
(function (backend) {
	...
	var defaultSortFucntion = function (column, a, b, sortType) {
		if (column && /^(asc|desc)$/i.test(sortType)) {
	...
	var defaultFilterFunction = function (column, data, value) {
		...
		if (!column.name)
			return true; // 没有列名，不做筛选
		if (Utils.isNull(data))
			return false;
		var _value = data[column.name];
		if (_value === value)
			return true;
		if (Utils.isNotNull(_value) && Utils.isNotNull(value)) {
			var dataType = column.dataType;
			if (/^(date|datetime|time)$/.test(dataType)) {
				value = Utils.toDate(value);
				if (!value)
				...
				// value = Utils.toDateString(value, "yyyy-MM-dd HH:mm:ss");
				// _value = Utils.toDateString(_value, "yyyy-MM-dd HH:mm:ss");
				// return Utils.toDateString()
			}
		}
		return _value == value;
	};
