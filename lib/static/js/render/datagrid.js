...
(function (backend) {
	...
	_Renderer._renderItems = function ($, target) {
		...
		// 如果筛选或排序，即渲染数据集与源数据集不同，需要更新选择索引
		if (hasFilterOrSort && (backend || !this.renderComplete)) {
			...
			if (selectedIndex && selectedIndex.length > 0) {
				...
				Utils.each(selectedIndex, function (index) {
					...
					if (data) {
						var _index = Utils.index(_datas, function (temp) {
							return temp == data;
						});
						if (_index >= 0)
							indexs.push(_index);
	...
	var renderItems = function ($, target, datas, indexs, selectedIndexs) {
		...
		Utils.each(datas, function (data, i) {
			...
			if (item) {
				// 后端渲染或者首次渲染，使用原始数据集索引，
				// 否则，由于前端的筛选、排序数据的实际索引已改变，此时以结果数据集的索引为依据（用户的角度是看到的列表的索引）
				var _index = (backend || !self.renderComplete) ? index : i;
				var bSelected = self._isSelected(data, _index, selectedIndex, selectedId);
