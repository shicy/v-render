...
(function (backend) {
	...
	var defaultSortIcons = {asc: "/VRender/icons/020c.png", desc: "/VRender/icons/021c.png"};
	...
	_Renderer._getSortFunction = function (column) {
		return getSortFunction.call(this, column);
	};
	...
	var renderSortView = function ($, toolbar, column) {
		var sortTarget = $("<div class='sort'></div>").appendTo(toolbar);
		var sortBtn = $("<i title='排序'></i>").appendTo(sortTarget);
		if (Utils.isArray(column.sortable) && column.sortable.length > 0) {
			renderToolDropdown.call(this, $, sortBtn, column.sortable, column.sortType, "无");
			// var dropdown = $("<ul class='dropdown'></ul>").appendTo(sortTarget);
			// Utils.each(column.sortable, function (temp) {
			// 	var type = Utils.trimToEmpty(temp.type);
			// 	if (type) {
			// 		var item = $("<li></li>").appendTo(dropdown);
			// 		item.attr("data-type", type);
			// 		var icon = temp.icon || defaultSortIcons[type];
			// 		if (icon)
			// 			$("<i></i>").appendTo(item).css("backgroundImage", "url(" + icon + ")");
			// 		$("<span></span>").appendTo(item).text(temp.label || "无");
			// 		if (type == column.sortType) {
			// 			item.addClass("selected");
			// 			if (icon)
			// 				sortBtn.css("backgroundImage", "url(" + icon + ")");
			// 			if (temp.label)
			// 				sortBtn.attr("title", temp.label);
			// 		}
			// 	}
			// });
		}
	...
	var renderFilterView = function ($, toolbar, column) {
		var filterTarget = $("<div class='filter'></div>").appendTo(toolbar);
		...
		// var dropdown = $("<ul class='dropdown'></ul>").appendTo(filterTarget);
		if (Utils.isArray(filterType)) {
			renderToolDropdown.call(this, $, filterBtn, filterType, column.filterValue, "空");
		}
		else {
	...
	var renderOthers = function ($, target, columns) {
		if (backend) {
			var _columns = Utils.map(columns, function (temp) {
				...
				if (Utils.isFunction(column.filter))
					column.filter = escape(column.filter);
		...
		return Utils.map(Utils.toArray(columns), function (column, i) {
			...
			if (Utils.isNull(column)) {
				...
				if (Utils.isTrue(column.sortable)) {
					_column.sortable = column.sortable;
					if (Utils.isFunction(column.sortFunction))
						_column.sortFunction = column.sortFunction;
					if (column.sortType) {
						_column.sortType = column.sortType;
						if (sortColumn)
							delete sortColumn.sortType;
						sortColumn = _column;
					}
					if (Utils.isArray(_column.sortable)) {
						_column.sortable = Utils.map(_column.sortable, function (tmp) {
							return {type: tmp.type, label: tmp.label, icon: tmp.icon, handler: tmp.handler, custom: tmp.custom};
						});
					}
					else if (!(Utils.isFunction(_column.sortable) || _column.sortable == "custom")) {
						_column.sortable = true;
					}
				}
				if (Utils.isTrue(column.filter)) {
					_column.filter = column.filter;
					if (Utils.isFunction(column.filterFunction))
						_column.filterFunction = column.filterFunction;
					if (Utils.isNotBlank(column.filterValue))
						_column.filterValue = column.filterValue;
					if (Utils.isArray(_column.filter)) {
						_column.filter = Utils.map(_column.filter, function (tmp) {
							return {label: tmp.label, value: tmp.value, icon: tmp.icon, handler: tmp.handler, custom: tmp.custom};
						});
					}
					else if (!(Utils.isFunction(_column.filter) || /^(custom|enum)$/.test(_column.filter))) {
						_column.filter = true;
					}
				}
			}
	...
	var getSortFunction = function (column) {
	...
	if (backend) {
		...
		Renderer.sortIcons = defaultSortIcons;
