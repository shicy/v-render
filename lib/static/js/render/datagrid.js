...
(function (backend) {
	...
	var renderFilterView = function ($, toolbar, column) {
		...
		renderFilterDropdownView.call(this, $, filterBtn, filterType);
		// if (Utils.isArray(filterType)) {
		// 	Utils.each(filterType, function (temp) {
		// 		var item = $("<li></li>").appendTo(dropdown);
		// 		item.attr("data-val", temp.value);
		// 		if (icon)
		// 			$("<i></i>").appendTo(item).css("backgroundImage", "url(" + icon + ")");
		// 		$("<span></span>").appendTo(item).text(temp.label || "空");
		// 		if (temp.value == column.filterValue) {
		// 			item.addClass("selected");
		// 			if (icon)
		// 				filterBtn.css("backgroundImage", "url(" + icon + ")");
		// 			if (temp.label)
		// 				filterBtn.attr("title", temp.label);
		// 		}
		// 	});
		// }
	};

	var renderFilterDropdownView = function ($, filterBtn, filterType) {
