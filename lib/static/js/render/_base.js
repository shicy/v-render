...
(function (backend) {
	...
	_BaseRenderer.renderData = function ($, target) {
		Fn.renderData.call(this, target, this.getData(true));
	};
	...
	// 初始化渲染组件选中项信息（索引或编号）
	_SelectRenderer.renderSelectValues = function ($, target) {
		var indexs = this.getSelectedIndex(true);
		target.attr("data-inds", indexs && indexs.join(","));

		var ids = this.getSelectedId(true);
		target.attr("data-ids", ids && ids.join(","));
