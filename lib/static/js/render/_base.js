...
(function (backend) {
	...
	ItemsRenderer.renderEmptyView = function ($, target, className) {
		...
		if (this.options.emptyView) {
			Fn.renderSubView.call(this, container, this.options.emptyView);
		}
		else {
			var emptyView = $("<div></div>").appendTo(container);
	...
	ItemsRenderer.renderLoadView = function ($, target, className) {
		...
		if (this.options.loadingView) {
			Fn.renderSubView.call(this, container, this.options.loadingView);
		}
		else {
	...
	SelectRenderer.getSelectedIndex = function (needArray) {
		...
		if (!this.isMultiple() && indexs.length > 1)
			indexs = [indexs[0]];
		if (indexs.length > 0)
			return needArray ? indexs : indexs[0];
		return needArray ? null : -1;
	};
	...
	SelectRenderer.getSelectedKey = function (needArray) {
		...
		if (!this.isMultiple() && ids.length > 1)
			ids = [ids[0]];
		if (ids.length > 0)
			return needArray ? ids : ids[0];
		return needArray ? null : -1;
	};

	// 获取选中的数据集
	SelectRenderer.getSelectedData = function (datas) {
		datas = datas || this.getData();
		var selectedDatas = [];
		if (datas && datas.length > 0) {
			var indexs = this.getSelectedIndex(true);
	...
	_SelectRenderer.getSelectedData = function (needArray) {
		var datas = SelectRenderer.getSelectedData.call(this);
	...
	// 判断是否全部选中
	_SelectRenderer.isAllSelected = function (datas) {
		datas = datas || this.getData();
		if (datas && datas.length > 0) {
			var indexs = this.getSelectedIndex(true);
			var ids = this.getSelectedKey(true) || [];
