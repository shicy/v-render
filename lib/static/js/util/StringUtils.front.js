...
(function (isFront) {
	var StringUtils = isFront ? VRender.Utils : exports;
	
	// 判断字符串是否以某个子串结尾
	StringUtils.endWith = function (str, sub) {
		if (isEmptyStr(str) || isEmptyStr(sub))
			return false;
	...
	// 获取字符串长度，checkChinese为true时中文字符算2个字符
	StringUtils.strLen = function (str, checkChinese) {
		if (isEmptyStr(str))
			return 0;
		if (checkChinese)
	...
	StringUtils.trimToUndefined = function (str) {
		return StringUtils.isBlank(str) ? undefined : ("" + str).trim();
	};
