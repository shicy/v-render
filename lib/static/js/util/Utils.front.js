...
(function (isFront) {
	...
	// 获取视图相对于浏览器窗口的位置
	Utils.getViewScreenOffset = function (target) {
		target = $(target);
		...
		offset.right = $(window).width() - x - target.outerWidth();
		offset.bottom = $(window).height() - y - target.outerHeight();
		return offset;
	};
	...
	// 获取元素相对容器的位置
	Utils.offsetContainer = function (target, container, targetWidth, targetHeight) {
		...
		var offset = target.offset() // 这是相对 body 的位置
		...
		offset.width = targetWidth >= 0 ? targetWidth : target.width();

		targetHeight = parseInt(targetHeight);
		offset.height = targetHeight >= 0 ? targetHeight : target.height();

		offset.overX = offset.left < 0 || (offset.left + offset.width > offset.containerWidth);
		offset.overY = offset.top < 0 || (offset.top + offset.height > offset.containerHeight);

		return offset;
	};

	Utils.offsetContainer_ = function (target, container, targetWidth, targetHeight) {
		if (!(target instanceof $))
			target = $(target);

		var offset = target.offset(); // 相对 body 的位置
		while (true) {
