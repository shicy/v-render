...
(function (isFront) {
	...
	Utils.debounce = function (handler, wait, params) {
		if (Utils.isFunction(handler)) {
			var item = Utils.find(debounceHandlers, function (temp) {
				return temp.handler == handler || temp.name == handler.handlerName;
			});
			if (!item) {
				item = {handler: handler, name: handler.handlerName, timerId: 0};
			...
			var self = this;
			var _params = item.params = Array.prototype.slice.call(arguments, 2);
	...
	Utils.offset = function (target, container, targetWidth, targetHeight) {
		...
		var offset = Utils.offsetToHtml(target); // {left, top}
		offset.width = parseInt(targetWidth);
		offset.width = offset.width >= 0 ? offset.width : target.width();
		offset.height = parseInt(targetHeight);
		offset.height = offset.height >= 0 ? offset.height : target.height();
		...
		if (container && container.length > 0) {
			var containerOffset = Utils.offsetToHtml(container); // {left, top}
