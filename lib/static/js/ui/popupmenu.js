...
(function () {
	...
	var onMoreClickHandler = function (e) {
		...
		params.pid = this._getDataId(parentData);
	...
	var doOpenAfter = function (item, container) {
		if (this.isRenderAsApp()) {
			...
			container.css("maxHeight", targetHeight);

			var offset = Utils.offsetWindow(container);
			offset.width = container.outerWidth();
			offset.height = container.outerHeight();
			...
			if (offset.top + offset.height > targetHeight) {
				if (isRootItem(item))
					container.css("maxHeight", targetHeight - offset.top);
				else
	...
	var doLoad = function (container, api, params, callback) {
		...
		Component.load.call(this, api, params, function (err, data) {
			...
			if (datas && datas.length > 0) {
				...
				if (self._totalSize > (self._pageSize * self._pageNo)) {
					datas.push([{__type__: "more", page: self._pageNo}]);
				}
