...
(function () {
	...
	_UICombobox._snapshoot_shoot = function (snapshoot) {
		snapshoot.selectedIndex = this.getSelectedIndex();
		snapshoot.selectedData = this.getSelectedData();
		snapshoot.value = this.$el.find(".ipt > input").val() || "";
	};

	_UICombobox._snapshoot_compare = function (snapshoot, selectedIndex) {
		var value = this.$el.find(".ipt > input").val() || "";
		if (snapshoot.value != value)
			return true;
		if (!selectedIndex)
			selectedIndex = this.getSelectedIndex(true);
		return !Component.list.equalIndex(selectedIndex, snapshoot.selectedIndex);
	};
