...
(function () {
	...
	_UICombobox._getNewItem = function ($, itemContainer, data, index) {
		if (this.isRenderAsApp() && this.isNative()) {
			...
			return $("<option></option>").appendTo(select);
		}
		else {
	...
	_UICombobox._loadBefore = function () {
		...
		if (this.isRenderAsApp() && this.isNative()) {
			...
			var target = itemContainer.parent();
			Renderer._item.renderLoadView.call(this, $, target);
		}
	};

	_UICombobox._loadAfter = function () {
		var itemContainer = this._getItemContainer();
		itemContainer.parent().find(".ui-load").remove();
		if (this.hasMore()) {
			if (this.isRenderAsApp() && this.isNative()) {
				...
				var target = itemContainer.parent();
				Renderer._item.renderMoreView.call(this, $, target);
			}
		}
	};
