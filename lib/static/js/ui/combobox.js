...
(function () {
	...
	UICombobox.instance = function (target) {
		return Component.instance(target, ".ui-combobox");
	};
	...
	_UICombobox._renderOneItem = function ($, item, data, index) {
		if (this.isRenderAsApp() && this.isNative()) {
			...
			Component.item.renderOneItem.call(this, item, null, data, index);
		}
	};
	...
	_UICombobox._rerender = function () {
		...
		Utils.debounce("combobox_render-" + this.getViewId(), function () {
			var input = self.$el.find(".ipt > input");
			...
			if (self.isEditable() && self.getSelectedIndex() < 0) {
				input.val(inputValue || "");
				setValueFlag.call(self, Utils.isNotBlank(inputValue));
			}
		});
	};

	_UICombobox._rerenderSilent = function () {
		var self = this;
		Utils.debounce("combobox_silent-" + this.getViewId(), function () {
			...
			if (selectedIndex >= 0) {
				var item = self._getItems().eq(selectedIndex);
				if (item && item.length > 0)
					setItemActive.call(self, item, true);
			}
		});
	};
