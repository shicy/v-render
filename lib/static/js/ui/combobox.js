...
(function (isFront) {
	...
	var iptClickHandler = function (e) {
		...
		if (this.isRenderAsApp()) {
			...
			// tap 在 foucsout 之前执行，这里要取消 focusout 事件，不然选项显示不了
			setTimeout(function () {
	...
	var dropdownScrollHandler = function (e) {
		var dropdown = this.$el.children(".ui-dropdownlist").children("ul");
	...
	// ====================================================
	var setActive = function (trigger, index, value) {
		var input = this.$el.find(".ipt > input");
		if (index >= 0) {
			input.val(value || this.dropdownList.getItemLabel(index));
		}
		else if (value || value === "") {
			input.val(value);
		}
		...
		if (Utils.isTrue(trigger)) {
			var data = this.dropdownList.getSelectedData(index);
	...
	var showDropdown = function () {
		...
		if (this.focusOutTimerId) {
			clearTimeout(this.focusOutTimerId);
			this.focusOutTimerId = 0;
		}
		...
		if (this.isRenderAsApp()) {
			...
			dropdown.children("ul").on("scroll.combobox", function (e) { dropdownScrollHandler.call(self, e); });
		}
		else {
	...
	var hideDropdown = function () {
		if (this.$el.is(".show-dropdown")) {
			...
			this.$el.find(".ui-dropdownlist ul").off("scroll.combobox");
		}
	};
