...
(function (isFront) {
	...
	_Holder.render = function ($, target) {
		...
		if (Utils.isTrue(this.options.readonly)) {
			target.addClass("readonly");
			input.attr("readonly", "readonly");
		}

		if (Utils.isTrue(this.options.required))
			target.attr("required", "required");

		var maxSize = parseInt(this.options.maxSize) || 0;
		if (maxSize > 0) {
		...
		if (Utils.isNotBlank(this.options.prompt))
			ipt.append("<span class='prompt'>" + this.options.prompt + "</span>");

		if (Utils.isNotBlank(this.options.tips))
			ipt.append("<span class='tips'>" + this.options.tips + "</span>");

		var description = this.options.description || this.options.desc;
	...
	_Holder.getWidth = function () {
		if (Utils.isBlank(this.options.width))
			return null;
		if (isNaN(this.options.width))
			return this.options.width;
		return (parseFloat(this.options.width) || 0) + "px";
	};

	_Holder.getDecimals = function () {
		if (this.isDisplayAsPassword())
			return 0;
		var type = this.options.dataType || this.options.type;
		if (type === "int")
			return 0;
		if (this.options.hasOwnProperty("decimals"))
			return parseInt(this.options.decimals) || 0;
		return 2;
	};
	...
	var renderInput = function ($, target, parent) {
		var multiple = Utils.isTrue(this.options.multi);
		...
		var iptValue = this.options.value;
		var type = this.options.dataType || this.options.type;
		if (Utils.isNotBlank(type)) {
			...
			if (/^(number|num|int)$/.test(type)) {
				...
				var minValue = parseFloat(this.options.min);
				var maxValue = parseFloat(this.options.max);
				if (!isNaN(minValue))
	...
	var UITextView = Component.TextView = function (view, options, holder) {
		...
		this.tap(".clear", function (e) { return clearBtnClickHandler.call(self, e); }, true);
	};
