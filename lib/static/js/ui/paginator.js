...
(function (isFront) {
	...
	_Holder.getTotalCount = function () {
		var totalCount = parseInt(this.options.total) || 0;
		return totalCount > 0 ? totalCount : 0;
	};

	_Holder.getPageNo = function () {
		var pageNo = parseInt(this.options.page) || 0;
		if (pageNo <= 1)
			return 1;
		var pageCount = this.getPageCount();
	...
	_Holder.getPageSize = function () {
		var pageSize = parseInt(this.options.size) || 0;
	...
	var renderStatus = function ($, target, pageNo, pageSize, pageCount, totalCount) {
		var statusPattern = this.options.status;
		if (statusPattern === true)
			statusPattern = "共{totalCount}条";
		else if (isFront)
			statusPattern = target.children(".status-src").text();
	...
	UIPaginator.create = function (options) {
		return Component.create(options, UIPaginator, Holder);
	};
