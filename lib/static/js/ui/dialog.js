...
(function (isFront) {
	...
	var Holder = function (options) {
		...
		this.setContentView(this.options.content || this.options.view);
	};
	...
	_Holder.getTitle = function () {
		return Utils.isBlank(this.d_title) ? "标题" : this.d_title;
	};
	_Holder.setTitle = function (value) {
		this.d_title = value;
	};

	_Holder.getContentView = function () {
		return this.d_contentview;
	};
	_Holder.setContentView = function (value) {
		this.d_contentview = value;
	};

	_Holder.getButtons = function () {
		if (Utils.isNull(this.d_buttons))
			return [{name: "submit", label: "确定"}, {name: "cancel", label: "取消"}];
		return Utils.toArray(ths.d_buttons);
	};
	_Holder.setButtons = function (value) {
		this.d_buttons = value;
	};
	...
	UIDialog.create = function (options) {
		...
		return new UIDialog(target, options);
	};
	...
	_UIDialog.load = function (pathname, params, callback) {
		...
		VRender.require(pathname, function (err, ret) {
			...
			if (contentView && contentView.length > 0) {
				contentView.on("submit submit_to_dialog", function (e) {
					self.trigger("view_submit");
					self.close();
				});
				self.on("btnclick", function (e, name, btn) {
					contentView.trigger("dialog_" + name);
