...
(function () {
	...
	var UIDialog = window.UIDialog = Component.Dialog = function (view, options) {
		...
		var activeBtn = this.$el.attr("opt-active");
		if (activeBtn) {
			this.$el.removeAttr("opt-active");
			activeBtn = this.activeBtn = $(activeBtn);
	...
	// 
	_UIDialog.waitting = function (waitFlag, btnName) {
		var button = null;
 ...
 //            pathname += (pathname.indexOf("?") < 0 ? "?" : "&") + $.param(params);
	...
	var windowResizeHandler = function () {
		if (this._isMounted()) {
			var container = this.$el.find(".dialog-view > section > .container");
			if (this.isRenderAsApp()) {
	...
	var setWaitting = function (waitTime, btn) {
		...
		if (waitTime) {
			if (button) {
				btn.addClass("waitting")
				button.waitting();
			}
			else {
				this.$el.addClass("waitting");
			}
			if (waitTime > 0) {
				var self = this;
				setTimeout(function () {
			...
			if (button) {
				btn.removeClass("waitting");
				button.waitting(false);
			}
			else {
