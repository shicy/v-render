...
(function () {
	...
	var UIDatagrid = window.UIDatagrid = Component.Datagrid = function (view, options) {
		...
		this.itemContainer.on("tap", "tr", itemClickHandler.bind(this));
		this.header.on("tap", "th.col-chk .chkbox", allChkboxClickHandler.bind(this));
	...
	var itemClickHandler = function (e) {
		...
		if (item.parent().is(this.itemContainer)) {
			if (item.is(".disabled"))
				return ;
			...
			var indexs = Utils.map(this.itemContainer.children(".selected"), function (item) {
				return item.index();
			});
		...
		if (this.isHeaderVisible()) {
			...
			Utils.each(cols, function (col, i) {
				...
				if (col.is(".is-expand")) {
					...
					var width = col.width();
					if (width != parseInt(header.attr("data-w")))
						header.attr("data-w", width).width(width);
				}
			});
		}
	};
