...
(function () {
	...
	_UIDatagrid.getSearcher = function () {
		return this.searcher;
	};
	_UIDatagrid.setSearcher = function (value) {
		if (this.searcher) {
			if (Utils.isFunction(this.searcher.off))
				this.searcher.off("search");
		}
		this.searcher = value;
		if (this.searcher && Utils.isFunction(this.searcher.on)) {
			var self = this;
			this.searcher.on("search", function (e, params) {
				if (self.lastLoadApi) {
					params = Utils.extend({}, Component.base.getInitParams.call(self), params);
	...
	var doInit = function () {
		...
		if (this.options.hasOwnProperty("searcher"))
			this.setSearcher(this.options.searcher);
		else {
			var searcher = this.$el.attr("opt-searcher");
			if (searcher) {
				searcher = $(searcher);
				if (searcher && searcher.length > 0) {
					searcher = Component.get(searcher) || VRender.FrontComponent.get(searcher) || searcher;
					this.setSearcher(searcher);
				}
			}
			this.$el.removeAttr("opt-searcher");
		}
