...
(function () {
	...
	var UIDatagrid = window.UIDatagrid = Component.Datagrid = function (view, options) {
		...
		if (this.isRenderAsApp()) {
			...
			this.$el.on("tap", ".sort-and-filter input", sortAndFilterInputClickHandler.bind(this));
			this.$el.on("tap", ".sort-and-filter .clearbtn", sortAndFilterClearHandler.bind(this));
			this.$el.on("tap", ".sort-and-filter .submitbtn", sortAndFilterSubmitHandler.bind(this));
			this.$el.on("keyup", ".sort-and-filter input", sortAndFilterInputKeyHandler.bind(this));
		}
		else {
	...
	var sortAndFilterClearHandler = function (e) {
		var input = this.$el.children(".sort-and-filter").find("input").val("");
		setTimeout(function () {
			input.focus();
		}, 0);
	};
