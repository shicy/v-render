...
(function () {
	...
	var UIDatagrid = window.UIDatagrid = Component.Datagrid = function (view, options) {
		...
		this.gridHead = this.$el.children(".table").children("header").children();
		this.gridBody = this.$el.children(".table").children("section").children();

		this.gridHead.on("tap", "th.col-chk", allChkboxClickHandler.bind(this));

		this.gridBody.on("tap", "tr", itemClickHandler.bind(this));
		this.gridBody.on("tap", "td.col-exp .expbtn", onExpandBtnHandler.bind(this));

		if (this.isRenderAsApp()) {
			this.gridHead.on("tap", "th", headClickHandler.bind(this));

			this.$el.on("tap", ".sort-and-filter", sortAndFilterClickHandler.bind(this));
			this.$el.on("tap", ".sort-and-filter .container", function () { return false; });
			this.$el.on("tap", ".sort-and-filter .item", sortAndFilterItemHandler.bind(this));
		}
		else {
			this.gridHead.on("tap", ".toolbar > *", toolbtnClickHandler.bind(this));
			this.gridHead.on("tap", ".toolbar .dropdown li", toolDropdownClickHandler.bind(this));
			this.gridHead.on("mouseenter", ".toolbar > *", toolbtnMouseHandler.bind(this));
			this.gridHead.on("mouseleave", ".toolbar > *", toolbtnMouseHandler.bind(this));
		}
	...
	_UIDatagrid.setExpandRenderer = function (value) {
		...
		if (_changed) {
			...
			expandRows.children(".col-exp").children(".expbtn").trigger("tap");
		}
	};
	...
	_UIDatagrid.addItem = function (data, index) {
		...
		if (index >= 0 && index < datas.length) {
			...
			Utils.each(this._getItems(), function (item) {
				var _index = parseInt(item.attr("opt-ind")) || 0;
				if (_index >= index)
					item.attr("opt-ind", (_index + 1));
			});
		}
		else {
			datas.push(data);
	...
	_UIDatagrid.updateItem = function (data, index) {
		...
		if (!index && index !== 0) {
			data = Component.list.doAdapter.call(this, data);
	...
	_UIDatagrid.removeItemAt = function (index) {
		...
		if (index >= 0) {
			...
			if (index < datas.length) {
				...
				var snapshoot = this._snapshoot();
				rerenderItems.call(this);
				this._dataChanged("delete", removedData, index);
				snapshoot.done();
			}
		}
	};
	...
	var headClickHandler = function (e) {
		...
		if (column) {
			if (column.filter || Utils.isArray(column.sortable))
				showSortAndFilterDialog.call(this, column);
	...
	var sortAndFilterItemHandler = function (e) {
		...
		if (item.parent().parent().is(".sort")) {
			...
			doFilter.call(this, column, item.attr("data-val"));
		}
		hideSortAndFilterDialog.call(this);
	};
	...
	var renderBySortAndFilter = function (hasFilterChanged) {
		...
		selectedChanged.call(this);
		if (hasFilterChanged) {
			Component.list.checkIfEmpty.call(this);
		}
	...
	// ====================================================
	// 显示 排序 和 筛选 对话框
	var showSortAndFilterDialog = function (column) {
		...
		if (isSortable)
			showDialogSortView.call(this, container, column);

		if (isFilterable)
			showDialogFilterView.call(this, container, column);
	...
	var showDialogSortView = function (container, column) {
	...
	var showDialogFilterView = function (container, column) {
	...
	var hideSortAndFilterDialog = function () {
		...
		var target = this.$el.children(".sort-and-filter").removeClass("inputing");
		setTimeout(function () {
