...
(function (isFront) {
	...
	_Holder.getSelectedIndex = function () {
		...
		if (options.hasOwnProperty("selectedIndexs")) {
			if (Utils.isBlank(options.selectedIndexs))
				return null;
			return Utils.toArray(options.selectedIndexs);
		}
		if (Utils.isNotBlank(options.selectedIndex))
			return Utils.toArray(options.selectedIndex);
		return null;
	};

	_Holder.getSelectedId = function () {
		var options = this.options || {};
		if (options.hasOwnProperty("selectedIds")) {
			if (Utils.isBlank(options.selectedIds))
				return null;
			return Utils.toArray(options.selectedIds);
		}
		if (Utils.isNotBlank(options.selectedId))
			return Utils.toArray(options.selectedId);
		return null;
	};
	...
	_Holder.getSelectedItems = function () {
		...
		if (options.hasOwnProperty("selectedItems")) {
			if (Utils.isBlank(options.selectedItems))
				return null;
			return Utils.toArray(options.selectedItems);
		}
		if (Utils.isNotBlank(options.selectedItem))
			return Utils.toArray(options.selectedItem);
		return null;
	};
	...
	UIDatagrid.create = function (options) {
		return Component.create(options, UIDatagrid, Holder);
	};
