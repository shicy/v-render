...
(function () {
	...
	var rerender = function () {
		DatagridRender.renderHeader.call(this, $, this.$el, this.__columns);
	...
	var renderBySortAndFilter = function (hasFilterChanged) {
		...
		var indexs = Utils.map(_datas, function (data) {
			return Utils.index(datas, function (temp) {
				return temp == data;
			});
		});
		var expandIndexs = Utils.map(this._getItems(".expand"), function (item) {
			return item.attr("opt-ind");
		});

		DatagridRender.renderItems.call(this, $, this.$el, _datas, indexs);
	...
	var selectedChanged = function () {
		...
		var indexs = Utils.map(this._getItems(".selected"), function (item) {
			// return parseInt(item.attr("opt-ind")) || 0;
			return item.index();
		});
