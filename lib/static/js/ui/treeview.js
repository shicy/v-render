...
(function () {
	...
	var _UITreeView = UITreeView.prototype = new Component.select();
		...
		if (pdata) { // 此时只修改 pdata 下的节点
			...
			if (Utils.isBlank(index)) {
				...
				var item = Utils.find(children, function (_item) {
					...
					return data == _data || dataId == self._getDataKey(_data);
				});
				if (item)
	...
	_UITreeView.load = function (api, params, callback) {
		...
		var _load = function (container, _params) {
			...
			doLoad.call(self, container, api, _params, function (err, datas) {
				...
				// 			_load(item, Utils.extend(params, {pid: getItemId.call(self, item)}));
				// 	}
				// }
	...
	var loadInner = function (container, api, params, callback) {
	...
	var tryAutoOpen = function () { // console.log(this.openProps);
		var item = null;
		var openProps = this.openProps || {};
		if (openProps.indexs && openProps.indexs.length > 0) {
			item = getItemByIndex.call(this, openProps.indexs.shift(), true);
		}
		else if (openProps.ids) {
		...
		if (item) {
			...
			if (!item.is(".is-loaded") && item.children("ul").children().length == 0) {
				...
				loadInner.call(this, item, null, params);
			}
			else {
	...
	_UITreeView._doAdapter = function (data) {
		...
		var _loopmap = function (_datas) {
			return Utils.map(_datas, function (_data, i) {
				_data = Renderer.fn.doAdapter.call(self, _data, dataIndex++);
	...
	_UITreeView._snapshoot_compare = function (state) {
		...
		return Renderer.fn.equalIndex(state.selectedIndex, selectedIndex);
	};
	...
	var doInit = function () {
		...
		if (!Component.isAutoLoad.call(this))
			tryAutoOpen.call(this);
	};
		...
		Component.load.call(this, api, params, function (err, data) {
			...
			if (datas && datas.length > 0) {
				...
				if (self.hasMore()) {
					...
					moreItem.attr("page-no", self._pageInfo.page);
				}
				else {
	...
	var getItemByData = function (data, deep) {
		...
		doLoop.call(this, function (item) {
			...
			if (!isMatch) {
				var _id = self._getDataKey(_data);
				isMatch = _id == dataId;
			}
			if (isMatch) {
				findItem = item;
