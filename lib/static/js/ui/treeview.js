...
(function () {
	...
	_UITreeView.addItem = function (data, pdata, index) {
		...
		var nodeIndex = container.is(".root") ? 0 : getItemIndex.call(this, container.parent());
	...
	var updateItem = function (item, data) {
		...
		var newItem = this._getNewItem($, container, data, nodeIndex);
		var nodeLevel = parseInt(container.attr("level"));
		TreeViewRender.renderOneNode.call(this, $, newItem, data, nodeIndex, nodeLevel);

		if (item.is(".selected"))
			newItem.addClass("selected");
		else if (item.is(".selected_"))
			newItem.addClass("selected_");

		if (item.children(".tree-node").is(".active"))
			newItem.children(".tree-node").addClass("active");
		...
		if (Utils.isArray(datas)) {
			datas.splice(newItem.index(), 1, data);
		}
	};
