...
(function (isFront) {
	...
	_Holder.render = function ($, target) {
		...
		var style = this.options.style || Holder.getTypeStyle(this.options.type);
		...
		if (Utils.isNotBlank(this.options.icon))
			mainBtn.append("<img class='icon' src='" + this.options.icon + "'/>");
		if (Utils.isNotBlank(this.options.label))
			mainBtn.append("<span class='lbl'>" + Utils.trimToEmpty(this.options.label) + "</span>");
