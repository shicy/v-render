...
(function (isFront) {
	...
	_HolderItems.getSelectedIndex = function (needArray) {
	...
	// 
	_HolderItems.getSelectedId = function (needArray) {
	...
	_HolderItems.getSelectedData = function (needArray) {
		var items = HolderItems.getSelectedItems.call(this, this.getData(), true);
		if (needArray || this.isMultiple())
			return items;
	...
	_HolderItems.renderOneData = function ($, itemTarget, data, index) {
	...
	_ComponentList.length = function () {
		return Utils.toArray(this.options.data).length;
	};
	...
	_ComponentList.getSelectedIndex = function (split) {
		var selectedIndex = this.$el.attr("data-inds") || null;
	...
	_ComponentList.setSelectedIndex = function (value) {
		if (Utils.isArray(value))
			value = value.join(",");
		this.$el.attr("data-inds", value || "");
	};

	_ComponentList.getSelectedId = function (split) {
	...
	_ComponentList.getSelectedData = function () {
