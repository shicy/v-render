...
(function (isFront) {
	...
	var renderQuickDates = function ($, target, quickDates, quickDefault) {
		if (quickDates && quickDates.length > 0) {
			...
			if (target.is(".tools-dropdown")) {
				if (items.children(".selected").length === 0)
					items.children(":eq(0)").addClass("selected");
			}
		}
	};
	...
	var UIDateRange = window.UIDateRange = Component.DateRange = function (view, options) {
		...
		this.tap(".ipt > .clear", function (e) { clearBtnHandler.call(self, e); }, true);
		this.tap(".tools > .item", function (e) { quickBtnHandler.call(self, e); });
		this.tap(".picker header button", function (e) { pickerMonthHandler.call(self, e); }, true);
		this.tap(".picker footer .ui-btn", function (e) { pickerButtonHandler.call(self, e); }, true);
		this.tap(".picker tbody td", function (e) { pickerDateHandler.call(self, e); }, true);
	...
	UIDateRange.create = function (options) {
		return Component.create(options, UIDateRange, Holder);
	};
