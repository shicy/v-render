...
(function () {
	...
	_UIFormView.getFormData = function () {
		...
		Utils.each(this._getItems(), function (item) {
			...
			if (contentView.is("input, textarea")) {
				...
				contentView = Component.get(contentView);
				if (contentView) {
	...
	_UIFormView.getActionBeforeHandler = function () {
		return Renderer.fn.getFunction.call(this, "actionBefore", "abefore");
	};
	_UIFormView.setActionBeforeHandler = function (value) {
		this.options.actionBefore = value;
		delete this.actionBefore;
		this.$el.children(".ui-fn[name='abefore']").remove();
	};
	...
	var doSubmit = function (callback) {
		...
		if (Utils.isBlank(action)) {
			...
			doSubmitBefore.call(this, params);

			var method = this.getMethod();
			if (/get/i.test(method)) {
				VRender.get(action, params, callback);
			}
			else if (/put/i.test(method)) {
				VRender.put(action, params, callback);
			}
			else if (/delete/i.test(method)) {
				VRender.delete(action, params, callback);
			}
			else {
				VRender.post(action, params, callback)
			}
		}
	};

	var doSubmitBefore = function (params) {
		var beforeHandler = this.getActionBeforeHandler();
		if (Utils.isFunction(beforeHandler)) {
			beforeHandler(params);
		}
		this.trigger("action-before", params);
	};
