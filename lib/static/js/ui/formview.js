...
(function () {
	...
	UIFormView.instance = function (target) {
		return Component.instance(target, ".ui-formview");
	};
	...
	_UIFormView.submit = function (action, callback) {
		...
		var self = this;
		var resultHandler = function (err, ret, submitParams) {
			...
			if (action) {
				self.trigger("action_after", err, ret, submitParams);
			}
		};
	...
	var validateItem = function (item, contentView, callback) {
		...
		if (contentView.is("input, textarea")) {
			...
			contentView = Component.get(contentView) || VRender.FrontComponent.get(contentView) || contentView;
	...
	var validateInterfaceView = function (item, view, value, callback) {
