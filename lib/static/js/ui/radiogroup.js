...
(function () {
	...
	UIRadioGroup.instance = function (target) {
		return Component.instance(target, ".ui-radgrp");
	};
	...
	_UIRadioGroup.setSelectedIndex = function (value) {
		...
		if (index >= 0) {
			Component.get(items.eq(index).children()).setChecked(true);
		}
		else {
			var selectedIndex = snapshoot.getState().selectedIndex;
			if (selectedIndex >= 0)
				Component.get(items.eq(selectedIndex).children()).setChecked(false);
		}
		snapshoot.done();
	};
