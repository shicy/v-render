...
(function () {
	...
	// ====================================================
	_UITabbar.setSelectedIndex = function (value, trigger) {
		var index = Utils.getIndexValue(value);
		if (index >= 0)
			this._getItems().eq(index).trigger("click");
	};
	...
	var itemClickHandler = function (e) {
		...
		if (changed) {
			Component.list.setSelectedIndex.call(this, item.index());
		...
		this.trigger("itemclick", item.attr("name"), item.index());
	...
	var closeInner = function (item, trigger) {
		...
		item.remove();
		Component.list.removeDataAt.call(this, temp1.index);
		...
		if (nextTab && nextTab.length > 0) {
			...
			temp2 = {index: nextTab.index(), name: nextTab.attr("name")};
			this.trigger("show", temp2.name, temp2.index);
		}

		var selectedItem = this.tabsView.find(".tab.selected");
		var setSelectedIndex = selectedItem && selectedItem.length > 0 ? selectedItem.index() : -1
		Component.list.setSelectedIndex.call(this, setSelectedIndex);
