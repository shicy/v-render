...
(function () {
	...
	UIDateInput.instance = function (target) {
		return Component.instance(target, ".ui-dateipt");
	};
	...
	var updatePicker = function (date, min, max) {
		...
		Utils.debounce("dateinput_picker-" + this.getViewId(), function () {
			if (self.isRenderAsApp() && self.isNative()) {
				var dateInput = self.inputTag.children(".origin-dateipt");
