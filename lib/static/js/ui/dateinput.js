...
(function (isFront) {
	...
	var UIDateInput = window.UIDateInput = Component.DateInput = function (view, options) {
		...
		if (!this.isRenderAsApp()) {
			...
			$("<input class='origin-dateipt' type='date'/>").appendTo(this.$el.children(".ipt"));
		}
	};
	...
	var iptClickHandler = function (e) {
		if (this.isRenderAsApp()) {
			// var dateInput = this.$el.children(".origin_dateipt");
			// if (!(dateInput && dateInput.length > 0)) {
			// 	dateInput = $("<input class='origin_dateipt' type='date'/>").appendTo(this.$el);
			// 	dateInput.on("change", originDateChangeHandler.bind(this));
			// }
			// // setTimeout(function () { dateInput.trigger("touchend"); }, 100);
			// dateInput.trigger("tap");
		}
		else {
