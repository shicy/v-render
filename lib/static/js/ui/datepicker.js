...
(function () {
	...
	var UIDatePicker = window.UIDatePicker = Component.DatePicker = function (view, options) {
		...
		if (this.isRenderAsApp()) {
			...
			this.tap("header .today", onTodayBtnHandler.bind(this));
			this.tap("header .clear", onClearBtnHandler.bind(this));
	...
	var rerender = function (pickerDate) {
		if (this.rerenderTimerId) {
			clearTimeout(this.rerenderTimerId);
		}
		var self = this;
		this.rerenderTimerId = setTimeout(function () {
			self.rerenderTimerId = 0;
