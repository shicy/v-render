...
(function () {
	...
	var UIPanel = window.UIPanel = Component.Panel = function (view, options) {
		...
		if (this.isRenderAsApp()) {
			...
			this.$el.on("tap", ".btnbar > .item > .btn", onButtonClickHandler.bind(this));
			this.$el.on("tap", ".btnbar > .item > ul > li", onDropdownButtonClickHandler.bind(this));
			this.$el.on("mouseenter", ".btnbar > .item", onButtonMouseHandler.bind(this));
			this.$el.on("mouseleave", ".btnbar > .item", onButtonMouseHandler.bind(this));

			this.$el.on("tap", ".tabbar > .tab", onTabClickHandler.bind(this));
		}
	};
	...
	_UIPanel.setViewports = function (value) {
	...
	var showViewport = function (name) {
		...
		viewports.removeClass("selected");
		if (viewport && viewport.length > 0) {
			viewport.addClass("selected");
		}
		else {
			viewports.eq(0).addClass("selected");
		}
	};
