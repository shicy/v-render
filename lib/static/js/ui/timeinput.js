...
(function () {
	...
	_UITimeInput._rerender = function () {
		var self = this;
		Utils.debounce("timeinput_render", function () {
			var time = self.getTime();
			setTimeInner.call(self, time);
			if (this.picker)
	...
	var showTimePicker = function () {
		...
		if (!picker || picker.length == 0) {
			picker = this.picker = $("<div class='picker'></div>").appendTo(this.$el);
	...
	var getPickerTime = function (picker) {
		...
		var time = [];
		// 时
		var hour = target.children(".hour").find(".selected").text();
		...
		// 分
		var minute = target.children(".minute").find(".selected").text();
		minute = parseInt(minute) || 0;
		time.push((minute < 10 ? "0" : "") + minute);
		// 秒
		if (this.isSecondVisible()) {
			var second = target.children(".second").find(".selected").text();
