...
(function () {
	...
	_ComponentBase._snapshoot = function () {
		...
		snapshoot.shoot = function (_state) {
			_state = _state || state;
			if (Utils.isFunction(self._snapshoot_shoot)) {
				self._snapshoot_shoot(_state);
			}
			else {
		...
		snapshoot.compare = function (args) {
			if (Utils.isFunction(self._snapshoot_compare)) {
				var params = [state].concat(Array.prototype.slice.call(arguments));
				return self._snapshoot_compare.apply(self, params);
			}
			else {
				this.shoot(newState);
		...
		snapshoot.release = function () {
			if (this._rootSnapshoot == this)
				this._rootSnapshoot = null;
		};
	...
	// 快照
	_ComponentList._snapshoot = function () {
		var snapshoot = {}, self = this;

		if (!this.rootSnapshoot)
			this.rootSnapshoot = snapshoot;

		snapshoot.refresh = function () {
			if (Utils.isFunction(self._snapshoot_shoot)) {
				self._snapshoot_shoot(snapshoot);
			}
			else {
				snapshoot.selectedIndex = self.getSelectedIndex();
				snapshoot.selectedData = self.getSelectedData();
			}
		};

		snapshoot.compare = function (selectedIndex) {
			if (Utils.isFunction(self._snapshoot_compare)) {
				return self._snapshoot_compare(snapshoot, selectedIndex);
			}
			else {
				if (!selectedIndex)
					selectedIndex = self.getSelectedIndex(true);
				return !ComponentList.equalIndex(snapshoot.selectedIndex, selectedIndex);
			}
		};

		snapshoot.done = function (selectedIndex, selectedData) {
			if (self.rootSnapshoot == snapshoot) {
				if (snapshoot.compare(selectedIndex)) {
					if (!selectedData)
						selectedData = self.getSelectedData();
					self.trigger("change", selectedData, snapshoot.selectedData);
				}
			}
			snapshoot.release();
		};
