...
(function () {
	...
	_ComponentBase.load = function (api, params, callback) {
		var self = this;
		return Fn.load(api, params, function (err, data) {
			if (!err) {
	...
	_ComponentBase.reload = function (callback) {
		return this.load(this.lastLoadApi, this.lastLoadParams, callback);
	};
	...
	ComponentItems.setData = function (value) {
	...
	ComponentItems.renderItems = function (itemContainer, datas) {
		return Renderer._item.renderItems.call(this, $, itemContainer, datas);
	};
	...
	// 添加列表项
	ComponentItems.addItem = function (data, index) {
	...
	// 更新列表项，index无效时将被忽略
	ComponentItems.updateItem = function (data, index) {
		data = Fn.doAdapter.call(this, data, index);
		if (!index && index !== 0)
	...
	// 删除列表项
	ComponentItems.removeItem = function (item, index) {
	...
	_ComponentItems.setData = function (value) {
		var snapshoot = this._snapshoot();
		ComponentItems.setData.call(this, value);
		snapshoot.done();
	};
	...
	_ComponentItems.getDataByKey = function (id) {
		return this.getDataAt(this.getIndexByKey(id));
	};

	_ComponentItems.getIndexByKey = function (id) {
		var datas = this.getData();
		if (datas && datas.length > 0) {
			var self = this;
			return Utils.index(datas, function (temp) {
				return self._getDataKey(temp) == id;
			});
		}
		return -1;
	};
	...
	_ComponentItems.setKeyField = function (value) {
		this.options.keyField = Utils.isBlank(value) ? "" : value;
	...
	_ComponentItems.more = function (callback) {
		if (this.lastLoadApi && this.hasMore() && !this.isLoading()) {
			...
			params.p_no = parseInt(this.pageInfo && this.pageInfo.page) || 0;
			params.p_no += 1;
	...
	_ComponentItems.hasMore = function () {
		if (this.pageInfo) {
			var page = parseInt(this.pageInfo.page) || 1;
			var size = parseInt(this.pageInfo.size) || 20;
			var total = parseInt(this.pageInfo.total) || 0;
	...
	ComponentSelect.getSelectedIndex = function (needArray) {
		...
		if (needArray || this.isMultiple())
			return indexs;
		return (indexs && indexs.length > 0) ? indexs[0] : -1;
	};
	...
	// 获取当前选中项对应的数据编号
	ComponentSelect.getSelectedId = function (needArray) {
		var self = this;
		var ids = this.$el.attr("data-ids");
		var indexs = this.$el.attr("data-inds");
		...
		if (Utils.isNull(ids))
			return null;
		var _ids = [];
		Utils.each((Utils.isArray(ids) ? ids : ids.split(",")), function (tmp) {
			if (tmp || tmp === 0)
				_ids.push(isNaN(tmp) ? tmp : parseInt(tmp));
		});
		if (needArray || this.isMultiple())
			return _ids;
		return _ids.length > 0 ? _ids[0] : null;
	};

	// 根据数据编号设置当前选中项
	ComponentSelect.setSelectedId = function (value) {
		if (!Utils.isArray(value))
	...
	// 获取当前选中项对应的数据
	ComponentSelect.getSelectedData = function (needArray) {
		var indexs = this.getSelectedIndex(true);
		var ids = (!indexs ? this.getSelectedId(true) : null) || [];
	...
	_ComponentSelect.setSelectedIndex = function (value) {
		return ComponentSelect.setSelectedIndex.call(this, value);
	};

	// 获取当前选中项对应的数据编号
	_ComponentSelect.getSelectedId = function (needArray) {
		return ComponentSelect.getSelectedId.call(this, needArray);
	};

	// 根据数据编号设置当前选中项
	_ComponentSelect.setSelectedId = function (value) {
		return ComponentSelect.setSelectedId.call(this, value);
	};
	...
	// 判断编号对应的项是否被选中
	_ComponentSelect.isSelectedId = function (value) {
	...
	var doInit = function (target, options) {
		...
		target.attr("ref", Utils.trimToNull(options.ref));
