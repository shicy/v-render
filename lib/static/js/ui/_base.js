...
(function () {
	...
	ComponentList.setPager = function (pager) {
		...
		if (this.pager && Utils.isFunction(this.pager.on)) {
			var self = this;
			this.pager.on("change", function (e, page, size) {
				var params = $.extend({}, self.lastLoadParams, {page: page});
				if (!page && Utils.isFunction(pager.getPage))
					params.page = pager.getPage();
				if (!size && Utils.isFunction(pager.getSize))
					params.size = pager.getSize();
				if (Utils.isFunction(self.setData))
					self.setData(null);
	...
	var tryAutoLoad = function () {
		if (isAutoLoad.call(this) && Utils.isFunction(this.load)) {
			...
			if (pager) {
				if (!params.page && Utils.isFunction(pager.getPage))
					params.page = pager.getPage();
				if (!params.size && Utils.isFunction(pager.getSize))
					params.size = pager.getSize();
			}
