...
(function () {
	...
	_ComponentList.setSelectedIndex = function (value) {
		var oldIndexs = this.getSelectedIndex(true);
		var oldDatas = this.getSelectedData();
		var indexs = ComponentList.setSelectedIndex.call(this, value);
		if (!ComponentList.equalIndex(indexs, oldIndexs)) {
			var items = this._getItems();
			if (items && items.length > 0) {
			...
			this.trigger("change", this.getSelectedData(), oldDatas);
		}
	};
