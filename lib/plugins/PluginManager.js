...
PluginManager.routeBefore = function (params, callback) {
	var self = this;
	doRouteLoop("routeBefore", function (handler, next) {
		if (handler == "end") {
			callback(); // 一个没有参数的回调方法，将继续执行默认路由
		}
		else {
			var result = false;
			...
			if (result !== true) {
				...
				callbackWarnTimerId = setTimeout(function () {
					console.warn("The routeBefore() function may not callback in plugin.");
				}, 2000);
			}
		}
	});
...
PluginManager.routeAfter = function (params, callback) {
	...
	var loop = function (index) {
		if (index < count) {
			...
			if (!plugin.target && Utils.isFunction(plugin.plugin.routeAfter)) {
				var result = false;
				var callbackWarnTimerId = 0;
				result = plugin.plugin.routeAfter(params.pathname, params.params, function (err, ret) {
					setTimeout(function () {
						if (callbackWarnTimerId)
							clearTimeout(callbackWarnTimerId);
						if (result === true) {
				...
				if (result === true) {
					callbackWarnTimerId = setTimeout(function () {
						console.warn("The routeAfter() function may not callback in plugin.");
					}, 2000);
				}
				else {
					loop(index + 1);
...
PluginManager.routeApi = function (name, params, callback) {
	...
	// 	if (index < count) {
	// 		var plugin = pluginList[index];
	// 		if (!plugin.target && Utils.isFunction(plugin.plugin.routeApi)) {
...
PluginManager.routeView = function (name, params, path, callback) {
...
var doRouteLoop = function (name, callback) {
