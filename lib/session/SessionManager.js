...
SessionManager.get = function (context, request) {
	...
	var sid = cookies["sid"];
	// console.log("000000000000>", request.url, sid, cookies.JSESSIONID);
	if (sid) {
	...
	if (cookies.JSESSIONID && cookies.JSESSIONID != session.jSessionID) {
		session.jSessionID = cookies.JSESSIONID;
		ArrayUtils.remove(session.cookies, (temp) => {
			return /^JSESSIONID/i.test(temp);
		});
		session.cookies.push(`JSESSIONID=${session.jSessionID}`)
	}
...
SessionManager.set = function (response, session) {
	...
	session.lastCookieSendTime = Date.now();
...
Session.prototype.initByJSESSIONID = function (value) {
	...
	this._userData = null;
	var cookie = ArrayUtils.find(this.cookies, function (tmp) {
...
Session.prototype.active = function () {
	this.lastActiveTime = Date.now();
	if (this.hasCookieSendToClient) {
		// 每小时更新一次cookie
		let lastCookieSendTime = parseInt(this.lastCookieSendTime) || 0;
		if (this.lastActiveTime - lastCookieSendTime > 60 * 60 * 1000)
			this.hasCookieSendToClient = false;
	}
};
