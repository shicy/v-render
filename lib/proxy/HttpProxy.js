...
var requestInner = function (method, session, server, url, params, callback) {
	...
	else if (/json/i.test(server.contentType)) {
...
var responseHandler = function (response, session, server, url, params, callback) {
	...
	response.on("end", function () {
		...
		if (status === HttpServer.StatusCode.NOTFOUND) {
			...
			if (server && server.contextPath && redirectUrl.indexOf(server.contextPath) == 0)
...
var getProxyHost = function (requestHost, userHost) { 
	...
	if (hosts) {
		...
		if (requestHost) {
			...
			if (requestHost != userHost) {
				if (!isIP(requestHost) && requestHost != "localhost")
					return requestHost;
			}
		}
		if (userHost && userHost != requestHost) {
