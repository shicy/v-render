...
var requestInner = function (method, session, server, url, params, callback) {
	...
	request.on("error", function (e) {
		if (e.code == "ENOTFOUND" || e.code == "ECONNREFUSED") { // 服务器不可用
			e = {code: -99999, msg: e.code, data: "服务器不可用"};
		}
...
var getProxyHost = function (requestHost, userHost) {
	...
	if (hosts) {
		if (requestHost && hosts.hasOwnProperty(requestHost)) {
		...
		if (isIP(requestHost) && !isIP(userHost)) {
			return userHost;
		}
	}
	return null;
