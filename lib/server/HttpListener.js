		...
		if (err) {
			...
			if (typeof ret === "object") {
				if (ret.status === VRender.RouterStatus.FILE)
					responseFile(ret.filepath, request, response);
					...
					responseJson(ret.data, response);
			}
			else {
				data = String(ret);
				var ext = Path.extname(data);
				if (ext && ext !== ".")
					responseFile(data, request, response);
				else
	...
	FileSys.exists(filepath, function (exists) {
		if (exists) {
			FileSys.stat(filepath, function (err, stat) {
				...
				// EasyWeb.logger.debug("<HttpServerListener.requestFile>", filepath, lastModified);
				...
				if (request.headers["if-modified-since"] == lastModified) {
					...
					self.writeFileData(response, filepath, encoding);
				}
			});
		}
		else {
			response.writeHead(HttpServer.StatusCode.NOTFOUND, {"Content-Type": ContentType.getTextContent()});
			response.write("This request URL has not found on the server.");
			response.end();
		}
	});
};
