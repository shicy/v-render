...
// 文件上传，按流的方式处理
HttpListener.prototype.doUpload = function (request, response) {
	var url = URL.parse(request.url, true);
	doRequest.call(this, request, response, url, url.query);
};
...
var doRequest = function (request, response, url, params) {
	var self = this;
	params = {requestData: Utils.extend({}, params)};
	...
	this.router.map(url, params, request, response, function (err, ret) {
		...
		if (err) {
			var responseType = ret && ret.responseType;
			if (!responseType) {
				if ([400, 401, 402, 403, 404, 500, 501, 502, 503, 504].indexOf(err.code) >= 0)
					responseType = err.code;
			}
			// 出错、报错、异常等情况，一般指非设计型错误，比如参数为空。
			// 比如需要会员才能访问的页面，而当前用户不是会员，这属于设计范围的错误，不应在这里处理
			responseError(err, response, responseType);
		}
		else if (Utils.notNull(ret)) {
			if (typeof ret !== "object")
				ret = {status: VRender.RouterStatus.OK, data: ret};
			if (ret.status === VRender.RouterStatus.FILE)
				responseFile.call(self, ret.filepath, params, request, response);
...
// 返回文件数据流
var responseFile = function (filepath, params, request, response) {
	var self = this;
		...
		if (exists) {
			FileSys.stat(filepath, function (err, stat) {
				...
				if (request.headers["if-modified-since"] == lastModified) {
					...
					writeFileData.call(self, response, params, filepath, encoding);
				}
			});
		}
		else {
			writeFileNotFound.call(self, response, filepath);
		}
	});
};

// 响应文件内容
var writeFileData = function (response, params, filepath, encoding) {
	params.filepath = filepath;
	const filename = params.filename || Path.basename(filepath);

	var stream = FileSys.createReadStream(filepath);

	var fileHandler = this.router.getFileHandler(params);
	if (fileHandler)
		stream = stream.pipe(fileHandler);

	var options = { "Content-Type": ContentType.get(filepath) };
	if (filename) {
	...
	if (encoding.match(/\bgzip\b/)) {
		...
		response.writeHead(HttpServer.StatusCode.OK, options);
