	...
	ArrayUtils.each(_imports, function (data) {
		...
		if (type == "script") {
			...
			if (!/\.(css|scss|sass)$/.test(filename))
				return ;
			if (isApp) {
				if (/\.p\.(css|scss|sass)$/.test(filename))
					return ;
			}
			else if (/\.m\.(css|scss|sass)$/.test(filename))
				return ;
		}
...
// var getMergedFilesInner0 = function (srcFiles, type) {
// 	console.log("111"); console.log(srcFiles);
...
// 			console.log("222", grp); console.log(mergeFiles.files);
...
// 		console.log("333"); console.log(results);
// 		return results;
// 	}
// 	else {
// 		return ArrayUtils.map(srcFiles, function (file) {
