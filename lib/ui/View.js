...
View.extend = function (module, props) {
	...
	child.prototype.__owner__ = child; // 相当于本类构造方法
...
View.import = function (filepath, attributes) {
	...
	ArrayUtils.each(ArrayUtils.toArray(filepath), function (path) {
		addOneImport(ViewClass, imports, path, attributes);
	});
};
...
View.prototype.use = function (dir, pathname) {
	var Module = require(getUseAbsolutePath(this.__owner__.__module__, dir, pathname));
...
View.prototype.setParent = function (parent) {
	...
	if (this.__parent__) {
		for (var i = this.__parent__.__children__.length - 1; i >= 0; i--) {
			if (this.__parent__.__children__[i] === this) {
				this.__parent__.__children__.splice(i, 1);
				break;
			}
		}
	}

	this.__parent__ = parent || null;
	if (this.__parent__) {
		if (!this.__parent__.__children__)
			this.__parent__.__children__ = [];
		this.__parent__.__children__.push(this);
	}
};
...
View.prototype.hasChild = function (view) {
	if (this.__children__) {
		for (var i = 0, l = this.__children__.length; i < l; i++) {
			if (this.__children__[i] === view)
				return true;
		}
	}
	return false;
...
///////////////////////////////////////////////////////////
var addOneImport = function (ViewClass, imports, filepath, attributes) {
	...
	else if (ViewClass && ViewClass.__module__ && !/^\[webroot\]/.test(filepath)) {
...
var doClassImport = function (imports, ViewClass, pushOnly) {
	if (ViewClass.__super__ && ViewClass.__super__.__owner__)
		doClassImport.call(this, imports, ViewClass.__super__.__owner__, pushOnly);
...
var getImportFiles = function (type) {
	...
	doClassImport.call(this, _imports, this.__owner__, true);
...
var getMergedFilesInner = function (srcFiles, type) {
	...
	if (Utils.isTrue(context.config("uglify"))) {
		...
		ArrayUtils.each(srcFiles, function (file) {
			...
			if (file.attributes)
				delete file.attributes.group;
			if (grp === false) { // 不合并，不压缩
				results.push({path: getSrcFileAbsolutePath(context, path),
					attributes: file.attributes, uri: getSrcFileURI(context, path)});
			}
			else {
		...
		for (var grp in groups) {
			...
			var minifierFile = FileMinifier.minifier(mergeFiles.files, type, expires);
			if (minifierFile) {
				var minifierSrc = [];
				ArrayUtils.each(mergeFiles.files, function (temp) {
					if (temp.uri)
						minifierSrc.push(temp.uri);
				});
				results.push({path: minifierFile, attributes: mergeFiles.attributes, uri: minifierSrc.join(";")});
			}
		}
...
var getSrcFileURI = function (context, path) {
	return /^\[webroot\]/.test(path) ? path.substr(9) : null;
};
