...
var UIView = View.extend(module, {
	...
	render: function (output) {
		...
		// if (Utils.isNotNull(width))
		// 	this.$el.css("width", width);
		// var height = this.getHeight();
		// if (Utils.isNotNull(height))
		...
		// this.$el.attr(this.getMapData(this.getViewData())); // 在render中处理
	...
	// 		if (!isNaN(width))
	// 			return Math.max(0, parseFloat(width)) + "px";
	// 	}
	// 	return null;
	// },
	...
	// getHeight: function () {
	// 	if (StringUtils.isNotBlank(this.options.height)) {
	// 		var height = this.options.height;
	// 		if (!isNaN(height))
	// 			return Math.max(0, parseFloat(height));
	// 	}
	// 	return null;
	// },
	...
	setEnabled: function (value) {
		this.options.disabled = Utils.isNull(value) ? false : !Utils.isTrue(value);
	},
