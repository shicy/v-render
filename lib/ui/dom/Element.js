...
Element.prototype.html = function (deep) {
	...
	if (StringUtils.isBlank(tagName)) {
		...
		if (StringUtils.isNotBlank(this.text))
			return this.needTextEncoding ? Utils.encodeHtml(this.text) : this.text;
		return "";
	}

	var html = "<" + tagName;
	var tagId = this.getId();
	...
	for (var n in attributes) {
		if (n === "id")
			continue;
		var value = StringUtils.trimToEmpty(attributes[n]);
		html += " " + n + "=\"" + Utils.encodeHtml(value) + "\"";
	}

	var styles = [];
	for (var n in this.styles) {
		if (StringUtils.isNotBlank(this.styles[n])) {
			var _n = n.replace(/([A-Z])/g, function (a) { return "-" + a.toLowerCase(); });
			styles.push(_n + ":" + this.styles[n]);
		}
	}
	if (styles.length > 0)
	...
	if (tagName === "meta")
		html += ">";
	else if (/^(img|link|br|input)$/.test(tagName))
		html += "/>";
	else {
		html += ">";
		var children = this.getChildren();
		if (deep && children && children.length > 0) {
			ArrayUtils.each(children, function (child) {
			...
			html += this.needTextEncoding ? Utils.encodeHtml(this.text) : this.text;
		}
		html += "</" + tagName + ">";
	}
	return html;
...
Element.prototype.innerHtml = function () {
	 ...
	 if (this.children && this.children.length > 0) {
	 	...
	 	html.push(this.needTextEncoding ? Utils.encodeHtml(this.text) : this.text);
	 return html.join("");
};
...
Element.prototype.getId = function () {
	var tagId = StringUtils.trimToNull(this.id);
	if (tagId === null && this.attributes)
		tagId = StringUtils.trimToNull(this.attributes.id);
	return tagId;
};
...
Element.prototype.getTagName = function () {
	return StringUtils.trimToEmpty(this.tagname);
};

Element.prototype.setTagName = function (value) {
	this.tagname = value;
};
...
// 设置节点文本内容，将会删除所有子节点
Element.prototype.setText = function (text, needEncoding) {
	if (Utils.notNull(text)) {
		this.removeChildren();
		this.text = text;
		this.htmlText = null;
		this.needTextEncoding = needEncoding;
	}
};
...
Element.prototype.removeClass = function (values) {
	if (ArrayUtils.isArray(this.clsnames) && this.clsnames.length > 0 && StringUtils.isNotBlank(values)) {
		if (ArrayUtils.isArray(values))
			values = values.join(" ");
		values = StringUtils.trimToEmpty(values).replace(/\s+/g, " ").split(" ");
...
Element.prototype.getStyle = function (name) {
	if (this.styles && StringUtils.isNotBlank(name)) {
		// name = name.replace(/([A-Z])/g, function (a) { return "-" + a.toLowerCase(); });
	...
	if ((child instanceof Element) && child != this) {
	 	...
	 	if (!this.children) {
	 		...
	 		if (StringUtils.isNotBlank(this.htmlText)) {
	 			...
	 			element.setHtml(this.htmlText);
	 			this.children.push(element);
	 			this.htmlText = null;
	 		}
	 		else if (Utils.notNull(this.text)) {
	 			var element = new Element();
	 			element.setText(this.text, this.needTextEncoding);
				this.children.push(element);
				this.text = null;
			}
	 	}
...
Element.prototype.clone = function () {
	...
	if (this.children && this.children.length > 0) {
		...
		newItem.needTextEncoding = this.needTextEncoding;
	}
	return newItem;
};
