...
// 动态创建一个元素标签
// 参数可以是一个选择器，如："div", "p", "#id", ".classname", "div#id.cls1.cls2"等
// 也可以是一个对象：{tag, id, cls|className, attrs|attributes, text, html}
exports.create = function (options) {
	if (options && StringUtils.isNotBlank(options)) {
		if (typeof options === "string") {
			if (/^#.+/.test(options)) { // #id, #id.cls, #id.cls1.cls2
				var temp = options.substr(1).split(".");
				options = {id: temp.shift(), cls: temp.join(" ")};
			}
			else if (/^\..+/.test(options)) // .cls, .cls1.cls2
				options = {cls: options.substr(1).split(".").join(" ")};
			else if (/^\<.+/.test(options)) // <div></div>
				options = {src: options};
			else { // div, div#id, div.cls, div#id.cls, div#id.cls1.cls2
				var temp = options.trim().split(".");
				var tagName = temp.shift().split("#");
				options = {tag: tagName[0], id: tagName[1], cls: temp.join(" ")};
			}
		}

		if (options.hasOwnProperty("src")) {
			if (StringUtils.isNotBlank(options.src)) {
				var element = HtmlParser.parse(options.src);
				return new DomHandler(null, element);
			}
		}
		else {
			var element = new Element((options.tag || "div"), (options.cls || options.className), 
				(options.attrs || options.attributs), options.id, options.text);
			if (StringUtils.isNotEmpty(options.html))
				element.setHtml(options.html);
			if (StringUtils.isNotEmpty(options.name))
				element.setAttribute("name", options.name);
			return new DomHandler(null, element);
		}
	}
	
	return new DomHandler(null, new Element("root-vrender"));
};

///////////////////////////////////////////////////////////
var DomHandler = function (options, elements) {
	this.VRENDERCLSID = "DomHandler";
...
DomHandler.prototype.prepend = function (value) {
	if (this.length > 0 && Utils.notNull(value)) {
		if (value instanceof DomHandler) {
			...
			appendElements(this, HtmlParser.parse(value), 0);
		}
	}
	return this;
};
...
DomHandler.prototype.remove = function (selector) {
	if (this.length > 0) {
		...
		ArrayUtils.each(this.elements, function (elem) {
			if (selector) {
				if (selector.length > 0 && isMatchSelector(elem, selector))
					elem.setParent(null);
			}
			else {
				elem.setParent(null);
...
// 删除绑定数据
DomHandler.prototype.removeData = function (value) {
	if (this.length > 0 && StringUtils.isNotBlank(value)) {
		var name = "data-" + value;
		ArrayUtils.each(this.elements, function (elem) {
...
// 读取或者设置元素的 value 值
DomHandler.prototype.val = function (value) {
	if (Utils.isNull(value)) {
		if (this.length > 0) {
...
var isMatchSelector = function (element, selector) {
	if (ArrayUtils.isArray(selector)) {
		...
		if (type === "[") {
			var temp = name.split("=");
			if (temp.length == 1)
				return element.hasAttribute(temp[0]);
			else
				return element.getAttribute(temp[0]) == temp[1].replace(/\'/g, "");
		}
		if (type === ":") {
			if (name === "empty") {
