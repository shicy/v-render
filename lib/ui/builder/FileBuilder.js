...
var doFilesBuild = function (context, fileGroups) {
	...
	ArrayUtils.each(fileGroups, function (group) {
		...
		if (group.files.length == 1 && group.files[0].search)
			targetFileUri += "?" + group.files[0].search;
...
var buildAsJsFile = function (context, filePath, fileId) {
	...
	if (buildResult.error) {
		console.error("buildAsJsFile: %s\n", filePath, buildResult.error);
		VRender.logger.error("<FileBuilder.buildAsJsFile> [%s] %s", filePath, JSON.stringify(buildResult));
	}
...
var sort = function(items) {
	...
	for (var i = 0; i < length - 1; i++) {
		...
		// 	console.log(items[j].uri, items[j].index);
		// }
		var temp = items[i];
		var index = parseInt(temp.index);
		var k = i + 1;
		for (var j = k; j < length; j++) {
			...
			if (temp.type == temp2.type) {
				...
				if (isNaN(index2))
					continue;
				if (!isNaN(index) && index <= index2)
					continue;
			}
			else if (temp.type == "css") {
				continue;
			}
			temp = temp2;
			index = parseInt(temp2.index);
			k = j;
		}
		if (items[i] != temp) {
			items.splice(k, 1);
