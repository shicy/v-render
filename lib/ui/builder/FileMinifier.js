...
var saveAsMinifierFile = function (pathname, minifierFileName, extname) {
	...
	if (extname === "css") {
		...
		minifyResult = minifyResult.replace(/\@charset\s\"utf-8\";/i, "");
		minifyResult = "@charset \"utf-8\";" + minifyResult;
	}
	else if (extname === "js") {
		var minifierName = Path.basename(minifierFileName).split(".")[0];
		minifyResult = UglifyJS.minify(minifyResult, {toplevel: true});
		if (minifyResult.error) {
			VRender.logger.error("<FileMinifier.saveAsMinifierFile>" + minifyResult.error);
		}
		else {
			minifyResult = minifyResult.code;
			if (/^define\(function\(/.test(minifyResult)) {
				minifyResult = minifyResult.replace(/^define\(function\(/, ("define('" + minifierName + "',function("));
			}
			else if (/^define\(\[/.test(minifyResult)) {
				minifyResult = minifyResult.replace(/^define\(\[/, ("define('" + minifierName + "',["));
			}
			else if (!/^define\(/.test(minifyResult)) {
				minifyResult = "define('" + minifierName + "',function($,VR,Utils){" + minifyResult + "});";
			}
		}
	}
