...
var PageView = View.extend(module, {
	...
	doFileImport: function () {
		...
		if (this.userState) {
			if (this.userState.isMobile) {
				this.import("/VRender.front.mobile.min.css?v=1703132157", {group: false, index: -1});
				this.import("/VRender.front.mobile.min.js?v=1703132157", {group: false, index: -1});
			}
			else {
				this.import("/VRender.front.min.css?v=1703132157", {group: false, index: -1});
				this.import("/VRender.front.min.js?v=1703132157", {group: false, index: -1});
			}
		}
	},
	...
	renderBody: function (body) {
		if (this.userState) {
			body.addClass(this.userState.isMobile ? "device-app" : "device-pc");
	...
	getMetaTypes: function () {
		var metas = [];
		if (this.userState && this.userState.isMobile) {
			metas.push({name: "viewport", content: "width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0"})
			// 全屏模式
