...
var PageView = View.extend(module, {
	...
	doFileImport: function () {
		...
		if (this.userState) {
			if (this.userState.isMobile) {
				this.import("/VRender.front.mobile.min.css?v=1711141228", {group: false, index: -1});
				this.import("/VRender.front.mobile.min.js?v=1711141228", {group: false, index: -1});
			}
			else {
				this.import("/VRender.front.min.css?v=1711141228", {group: false, index: -1});
				this.import("/VRender.front.min.js?v=1711141228", {group: false, index: -1});
			}
		}
	},
	...
	renderHeader: function (head) {
		...
		// 添加样式
		ArrayUtils.each(this.getMergedStyleFiles(this.isRenderAsApp()), function (file) {
